...
public class Ti994LoaderHelper {
    ...
    static void commentFiadOrTifilesHeader(Field[] fields, Program program, Address headerAddress, ByteProvider provider) throws Exception {
        ...
        for (Field field : fields) {
            ...
            if (field == Field.FILE_STATUS_FLAGS) {
            	long sfOff = ha.getOffset();
            	int sf = reader.readByte(sfOff) & 0xff;
                Map.Entry<String, String> typePair = parseStatusFlags(sf);
                statusFlagType = typePair.getKey();
                statusFlagExtra = typePair.getValue();
                comment += "\nType: " + statusFlagType;// + (statusFlagExtra != null ? " (" + statusFlagExtra + ")" : "");
                if (!statusFlagExtra.isEmpty()) {
                    comment += " (" + statusFlagExtra + ")";
                }
            } else if (field == Field.LOGICAL_RECORD_LENGTH) {
                long lrOff = ha.getOffset();
                int lrl = reader.readByte(lrOff) & 0xff;
        ...
        if (statusFlagType != null) {
            // if we're using fixed size records, include the size in the type string
            if (statusFlagType.substring(4, 7).equals("FIX")) {
                if (logicalRecordLength != null) {
                    appendComment(listing, headerAddress, CodeUnit.PRE_COMMENT, statusFlagType + " " + logicalRecordLength);
                } else {
                    appendComment(listing, headerAddress, CodeUnit.PRE_COMMENT, statusFlagType + " ???");
                }
            } else {
                appendComment(listing, headerAddress, CodeUnit.PRE_COMMENT, statusFlagType);
            }
    ...
    static Map.Entry<String, String> parseStatusFlags(int flags) {
        ...
        switch (type) {
            ...
            case INT_VAR:
                ...
                break;
            default:
                typeString += "???"; // PROG + DIS/INT/FIX/VAR set
        }
    ...
    static void commentFiadHeader(Program program, Address headerAddress, ByteProvider provider) throws Exception {
    	commentFiadOrTifilesHeader(new Field[] {
            ...
            Field.NUMBER_OF_LEVEL_3_RECORDS_ALLOCATED,
            Field.FIAD_FILLER
        }, program, headerAddress, provider);
    }

    static void commentTiFilesHeader(Program program, Address headerAddress, ByteProvider provider) throws Exception {
    	commentFiadOrTifilesHeader(new Field[] {
            Field.TIFILES_MAGIC,
            Field.NUMBER_OF_SECTORS_CURRENTLY_ALLOCATED,
            Field.FILE_STATUS_FLAGS,
    ...
    static void commentStandardHeader(Program program, Address addr, BinaryReader reader, int startIndex) throws CodeUnitInsertionException, IOException {
        ...
        if (pointerToProgList != 0) {
            Address firstProg = addr.getAddressSpace().getAddress(pointerToProgList);
            ...
            // TODO add a label GPL_entry for the first program
