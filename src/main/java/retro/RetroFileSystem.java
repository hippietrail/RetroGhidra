...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = RetroFileSystem.MyFileSystemFactory.class)
public class RetroFileSystem implements GFileSystem {
	 ...
	 */
	public RetroFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + RetroFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<RetroFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public RetroFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			RetroFileSystem fs = new RetroFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
